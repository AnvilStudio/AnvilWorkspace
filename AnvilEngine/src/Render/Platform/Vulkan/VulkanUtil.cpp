...
namespace anv::vk_util
{
        ...
        VkDeviceMemory& bufferMemory)
    {
        ...
        ANV_VK_CHECK_RESULT(
            vkCreateBuffer(_dev, &bufferInfo, nullptr, &buffer),
            "Failed to create buffer!"
        );

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(_dev, buffer, &memRequirements);

        VkMemoryAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocInfo.allocationSize = memRequirements.size;
        allocInfo.memoryTypeIndex = vku_FindMemoryType(
            _pdev,
        ...
        ANV_VK_CHECK_RESULT(
            vkAllocateMemory(_dev, &allocInfo, nullptr, &bufferMemory),
            "Failed to allocate buffer memory!"
        );

        vkBindBufferMemory(_dev, buffer, bufferMemory, 0);
    }
    ...
    uint32_t vku_FindMemoryType(VkPhysicalDevice _dev, uint32_t typeFilter, VkMemoryPropertyFlags properties)
    {
        ...
        vkGetPhysicalDeviceMemoryProperties(_dev, &memProperties);
        ...
        ANV_LOG_FATAL("Failed to find suitable memory type!");
        return 0;
    }
