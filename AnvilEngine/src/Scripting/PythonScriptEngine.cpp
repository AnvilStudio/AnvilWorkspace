...
namespace anv
{
    namespace
    {
        ...
        PyObject *load_module(const std::filesystem::path &_path, ModuleRecord &_record)
        {
            ...
            PyObject *spec = PyObject_CallMethod(importlib, "spec_from_file_location", "ss", name.c_str(), _path.string().c_str());
            Py_DECREF(importlib);

            if (!spec)
                return nullptr;

            PyObject *module = PyObject_CallMethod(PyImport_ImportModule("importlib.util"), "module_from_spec", "O", spec);
            if (!module)
            {
                Py_DECREF(spec);
                return nullptr;
    ...
    bool PythonScriptEngine::Initialize(const std::filesystem::path &_projectDirectory)
    {
        ...
        {
            ANV_LOG_ERROR("Anvil Python API directory was not found: '%s'", s_PythonModuleDirectory.string().c_str());
            return false;
        }
    ...
    bool PythonScriptEngine::IsInitialized() { return s_Initialized; }
